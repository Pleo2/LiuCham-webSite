			...
			<article className="w-full flex flex-col justify-center items-center mt-4 gap-12 sm:mt-8 lg:flex-row ">
				{!error && !loading ? (
					data?.map((tour: DataTypeTours, index: number) => {
						return (
							<article key={tour?.id} className='sm:bg-custom-greenFalse  sm:rounded-sm sm:p-4 sm:flex sm:gap-4 sm:justify-center sm:items-center lg:min-h-80 lg:h-80 lg:w-ful'>
								<Image
									priority={true}
									src={
								...
								/>
								<div>
									<div className="text-3xl font-display mt-2 text-white sm:text-custom-fullBlack lg:w-80">{tour?.title}</div>
									<div className="bg-transparent mt-2 font-sans text-xs text-custom-whitePalo sm:text-custom-fullBlack lg:w-80">
